...
app.factory('auth', function($http, $q, identity, authorization, UsersResource, baseUrl) {
    ...
    return {
        ...
        login: function(user){
            ...
            $http.post(baseUrl + '/token', 'username=' + user.username + '&password=' + user.password + '&grant_type=password', { headers: {'Content-Type': 'application/x-www-form-urlencoded'} }).success(function(response) {
